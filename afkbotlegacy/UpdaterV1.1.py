...
url = 'https://github.com/gorouflex/afkbotlegacy/releases/latest/download/AFKBotLegacy.zip'
...
def download_file():
    r = requests.get(url)
    with open('AFKBotLegacy.zip', 'wb') as f:
        f.write(r.content)
    with zipfile.ZipFile('AFKBotLegacy.zip', 'r') as zip_ref:
        zip_ref.extractall()
    caution_label.configure(text="Update successfully!")
    os.remove("AFKBotLegacy.zip")


root = ctk.CTk()
root.geometry("400x90")
root.title("AFKBot Legacy Updater V1.1")
